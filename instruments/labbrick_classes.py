...
================================
​
:Authors: David Schuster, Andrew Rockovich
"""
...
# try:
#     LDA602dllpath = r'C:\_Lib\python\malab\instruments\VNX_atten.dll'
#     LDA602DLL = C.CDLL(LDA602dllpath)
# except:
#     print("Warning could not load LDA602 labbrick dll, check that dll located at '%s'" % LDA602dllpath)
#
# try:
#     # LMS103dllpath=r'S:\_Lib\python\scratch\labbrick\vnx_fmsynth.dll'
#     LMS103dllpath = r'C:\_Lib\python\malab\instruments\vnx_fmsynth.dll'
...
try:
    # LMS103dllpath=r'S:\_Lib\python\scratch\labbrick\vnx_fmsynth.dll'
    LPSdllpath = r'C:\_Lib\python\malab\instruments\VNX_dps.dll'
    LPSDLL = C.CDLL(LPSdllpath)
except:
    print("Warning could not load LPS labbrick dll, check that dll located at '%s'" % LPSdllpath)

# Adding digital attenuator:
try:
    LDA5018path = r'C:\_Lib\python\malab\instruments\VNX_atten64.dll'
    LDA5018DLL = C.CDLL(LDA5018path)
except:
    print("Warning could not load LDA5018 labbrick dll, check that dll located at '%s'" % LDA5018path)
...
#THIS DOES NOT INCLUDE GETMODELNAME. IT IS INTERCHANGEABLE WITH SERIAL NUMBER ANYWAYS:
def LDA5018_get_device_info():
    """
    Returns a dictionary of device information

    :returns: {'model': Model name, 'serial':Serial number, 'devid':Device ID}
    """
    dll = LDA5018DLL
    dll.fnLDA_SetTestMode(U8(int(False)))
    device_info_array_type = U32 * int(dll.fnLDA_GetNumDevices(None))
...
#         serial = int(serial)
#         devinfos = LDA602_get_device_info()
...
# Add a class for the LDA5018 digital attenuator:
class LDA5018(Instrument):
    'The interface to the Lab Brick Digital Attenuator'
    dll = LDA5018DLL

    # Address is serial number, specify LDA5018(address=*serial number of device here*) when creating class instance!!
    def __init__(self, name='LDA5018', address=None, enabled=True):
        Instrument.__init__(self, name, address, enabled=True)
        self.set_test_mode(False)
        if address is not None:
            self.init_by_serial(int(address))
    ...
    def get_info_by_serial(self, serial):
        serial = int(serial)
        devinfos = LDA5018_get_device_info()
        for devinfo in devinfos:
            # print devinfo
            if devinfo['serial'] == serial:
                return devinfo
...
#HARD CODED IN MODEL NAME BECAUSE GETMODELNAME() IS NOT WORKING.
    def get_model_name(self):
        model_name = 'LDA-5018V'
        return model_name
    ...
    def init_device(self):
        self.dll.fnLDA_InitDevice(self.devid)
#Manually put the model in, because GetModelName does not function properly
    def get_id(self):
        return "Model: %s serial #: %d" % ('LDA-5018V', self.devinfo['serial'])

    def set_attenuation(self, attenuation):
        """
        :param attenuation: Attenuation in .25dB units, may range from 0 dB to 50 dB in steps of 0.5 dB.
        :return:
        """
        if attenuation > 50 or attenuation < 0:
            print("%.2f dB falls outside the range (0 - 50 dB). Setting to attenuation to 50 dB" % attenuation)
            attenuation = 50
    ...
    def get_rf_on(self):
        ...
        """
        return float(self.dll.fnLDA_GetRF_On(self.devid))
    # Can define methods for ramping, but we will not be using the ramping function
...
class LPS802(Instrument):
    ...
    # GetModelName doesn't work, hard code in the model name
    def get_model_name(self):
        model_name = 'LPS-802'
        return model_name
    ...
    def get_id(self):
        return "Labbrick Phase Shifter model: %s serial #: %d" % ('LPS-802', self.devinfo['serial'])
...
if __name__ == "__main__":
    ...
    #-----------------------------------------------------------------------------------------------------------------------
    # #Test the Digital attenuator class and methods:
    # vnx = LDA5018(address=22084)    #create instance of LDA5018 class (address is serial number)
    # #vnx.init_device()              #Manually initialize the device (Creating instance of class object LDA5018
    #                                     # automatically initializes device, so this is unnecessary)
    # print(vnx.get_id())             #Return the model name and serial number of device
    # print(vnx.get_num_devices())    #Return number of lab brick devices connected
    # #vnx.get_model_name()           #Return model name of device (Not working for digital attenuator
    # vnx.set_test_mode()             #default False (test mode off), So no need to call this
    # vnx.set_attenuation(0)         #Set the attenuation (in dB, increments of .5dB) (Max is 50dB, anything above results in
    #                                     # 50dB of attenuation).
    # # vnx.set_rf_on(1)              #True = set attenuation, False = maximum attenuation (by using vnx.set_attenuation(large number)
    #                                     # the attenuation automatically sets to the maximum, 50dB, so rf can be avoided
    # print(vnx.get_rf_on())          #True = attenuator is on (at set attenuation), False = Maximum attenuation
    # print(vnx.get_attenuation())    #Print the attenuation, in d
    # vnx.close_device()              #Make sure to close the interface with device
    #------------------------------------------------------------------------------------------------------------------------
    # #Test the Phase shifter class and methods:
    lps = LPS802(address = 22099)     #create instance of LPS802 class (address is serial number)
    print(lps.get_id())               #Print ID of device
    print(lps.get_model_name())
    print(lps.get_num_devices())
    print(lps.get_min_working_frequency())
    print(lps.get_max_working_frequency())
    lps.set_working_frequency(5e9)    #Set working frequency, in Hz
    lps.set_phase(360)                #angle in degrees of phase shift. second argument is optional (resets the working
                                            # frequency, if not specified working frequency is unchanged)
    lps.close_device()
